    ...
    const fetchDynamicImages = async () => {
      try {
        const carouselPromises = [1, 2, 3, 4, 5].map(num =>
          axios.get(`${apiBaseUrl}/api/content/program_fundamentals_carousel_${num}`)
        );
        const internalPromise = axios.get(`${apiBaseUrl}/api/content/program_fundamentals_internal_1`);
